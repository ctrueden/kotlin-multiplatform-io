...
class ApngTest {
    ...
    val String.path
        get() = HostFileSystem.canonicalize(".".toPath()) / "build/resources/apng/$this.png"

    @Test
    fun `scifio-test`() {
        val meta = APNGFormat.Metadata()
        HostFileSystem.openReadOnly("scifio-test".path).use { ApngParser().typedParse(it, meta) }
        meta.apply {
            idat.size `==` 9
            idat[0].apply {
                offset `==` 136L
